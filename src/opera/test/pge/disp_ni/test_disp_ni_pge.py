...
import opera.pge.disp_ni.disp_ni_pge
...
class DispNIPgeTestCase(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        dummy_input_files = ['compressed_slc_t087_185678_iw2_20180101_20180210.h5',
                             ...
                             'opera-ni-disp-frame-to-burst.json']
    ...
    @patch.object(opera.pge.disp_ni.disp_ni_pge.subprocess, "run", mock_grib_to_netcdf)
    def test_disp_ni_pge_execution(self):
        ...
        # Kickoff execution of DISP-NI PGE
        pge.run()
        ...
        expected_inter_filename = abspath("disp_ni_pge_test/output_dir/20180101_20180330.unw.nc")
    ...
    def test_filename_application(self):
        ...
        expected_disp_filename = pge._netcdf_filename(
            inter_filename=abspath("disp_ni_pge_test/output_dir/20180101_20180330.unw.nc")
        )

        self.assertEqual(os.path.basename(nc_file), expected_disp_filename)

        disp_metadata = get_disp_ni_product_metadata(nc_file)
    ...
    def test_iso_metadata_creation(self):
        ...
        disp_metadata = pge._collect_disp_ni_product_metadata(disp_metadata_path)
    ...
    def test_bad_algorithm_parameters_schema_path(self):
        ...
        """
        runconfig_path = join(self.data_dir, 'test_disp_ni_config.yaml')
        test_runconfig_path = join(self.data_dir, 'invalid_disp_ni_config.yaml')
    ...
    def test_disp_ni_pge_validate_inputs(self):
        ...
        frame_to_burst_file = 'opera-ni-disp-frame-to-burst.json'
        ...
        try:
            ...
            runconfig_dict['RunConfig']['Groups']['PGE']['PrimaryExecutable']['ProgramOptions'] = \
                ['-p disp_ni_pge_test/output_dir/compressed_slcs;',
                 'dd if=/dev/urandom of=disp_ni_pge_test/output_dir/20180101_20180330.unw.nc bs=1M count=1;',
                 'dd if=/dev/urandom of=disp_ni_pge_test/output_dir/20180101_20180331.unw.nc bs=1M count=1; echo ']
            ...
            runconfig_dict['RunConfig']['Groups']['PGE']['PrimaryExecutable']['ProgramOptions'] = \
                ['-p disp_ni_pge_test/output_dir/compressed_slcs;',
                 'touch disp_ni_pge_test/output_dir/20180101_20180330.unw.nc; echo ']
            ...
            runconfig_dict['RunConfig']['Groups']['PGE']['PrimaryExecutable']['ProgramOptions'] = \
                ['-p disp_ni_pge_test/output_dir/compressed_slcs;',
                 'dd if=/dev/urandom of=disp_ni_pge_test/output_dir/20180101_20180330.unw.nc bs=1M count=1;',
                 'touch disp_ni_pge_test/output_dir/20180101_20180330.unw.png;',
                 '/bin/echo DISP-NI invoked with RunConfig']
            ...
            runconfig_dict['RunConfig']['Groups']['PGE']['PrimaryExecutable']['ProgramOptions'] = \
                ['-p disp_ni_pge_test/output_dir/compressed_slcs;',
                 'dd if=/dev/urandom of=disp_ni_pge_test/output_dir/20180101_20180330.unw.nc bs=1M count=1;',
                 'dd if=/dev/urandom of=disp_ni_pge_test/output_dir/20180101_20180330.unw.png bs=1M count=1;',
                 'touch disp_ni_pge_test/output_dir/compressed_slcs/compressed_slc_t087_185684_iw2_20180222_20180330.h5;',   # noqa E501
                 '/bin/echo DISP-NI invoked with RunConfig']
    ...
    def test_scratch_sas_runconfig_for_grib_to_netcdf_files(self):
        """
        Test that the grib_to_netcdf files are in 'disp_ni_pge_test/scratch_dir'.
        ...
        temp_sas_runconfig = 'disp_ni_pge_test/scratch_dir/' + pge.runconfig.filename.split('/')[-1][:-5] + '_sas.yaml'
        # open the yaml file
