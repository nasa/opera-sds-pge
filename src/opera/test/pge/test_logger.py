...
from opera.util.logger import save_stream
...
class LoggerTestCase(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.reps = 1
        cls.severity_summary = {'Debug': 3, 'Info': 31, 'Warning': 3, 'Critical': 2}
    ...
    def test_save_stream(self):
        myStream = StringIO("Baseball has divinely inspired geometry.")
        save_stream(myStream, "saveStream.txt")
    ...
    def test_pge_logger(self):
        ...
        # self.assertEqual(self.logger.log_file.name, re.match(self.fn_regex, self.logger.log_file.name).group())
        ...
        self.assertEqual(self.logger.log_count_by_severity, {'Debug': 0, 'Info': 0, 'Warning': 0, 'Critical': 0})
    ...
    #
        ...
        with open('new_file.txt', 'w') as tempfile:
            tempfile.write('Text from "new_file.txt" to test append_text_from another file().')
        ...
        self.assertIn('Moving log file from ', log)
        ...
        self.assertEqual(self.logger.log_count_by_severity, self.severity_summary)
    ...
    #     self.logger.log_one_metric('test_logger.py', 'Test log_one_metric() with back frame starting at 1',
    #                                18, additional_back_frames=1)
    #     self.test_one_deeper()
    #
    # def test_one_deeper(self):
    #     self.logger.log_one_metric('test_logger.py', 'Test log_one_metric() with back frame starting at 2',
    #                                19, additional_back_frames=2)
    #
    def cleanup(self):
