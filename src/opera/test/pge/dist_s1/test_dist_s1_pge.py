...
class DistS1PgeTestCase(unittest.TestCase):
    ...
    def _compare_algorithm_parameters_runconfig_to_expected(self, runconfig):
        ...
        self.assertEqual(runconfig['algo_config']['low_confidence_alert_threshold'], 2.5)
        self.assertEqual(runconfig['algo_config']['high_confidence_alert_threshold'], 4.5)
        self.assertEqual(runconfig['algo_config']['device'], 'cpu')
        self.assertTrue(runconfig['algo_config']['apply_despeckling'])
        self.assertTrue(runconfig['algo_config']['apply_logit_to_inputs'])
        self.assertEqual(runconfig['algo_config']['memory_strategy'], 'high')
        self.assertEqual(runconfig['algo_config']['batch_size_for_norm_param_estimation'], 32)
        self.assertEqual(runconfig['algo_config']['stride_for_norm_param_estimation'], 7)
        self.assertEqual(runconfig['algo_config']['n_workers_for_despeckling'], 8)
        self.assertEqual(runconfig['algo_config']['n_workers_for_norm_param_estimation'], 4)
        self.assertFalse(runconfig['algo_config']['tqdm_enabled'])
        self.assertFalse(runconfig['algo_config']['model_compilation'])
        self.assertEqual(runconfig['algo_config']['lookback_strategy'], 'multi_window')
        self.assertEqual(runconfig['algo_config']['post_date_buffer_days'], 1)
        self.assertEqual(runconfig['algo_config']['max_pre_imgs_per_burst_mw'], [3, 3, 4])
        self.assertEqual(runconfig['algo_config']['delta_lookback_days_mw'], [1095, 730, 365])
        self.assertEqual(runconfig['algo_config']['no_day_limit'], 30)
        self.assertEqual(runconfig['algo_config']['exclude_consecutive_no_dist'], 1)
        self.assertEqual(runconfig['algo_config']['percent_reset_thresh'], 50)
        self.assertEqual(runconfig['algo_config']['no_count_reset_thresh'], 10)
        self.assertEqual(runconfig['algo_config']['max_obs_num_year'], 253)
        self.assertEqual(runconfig['algo_config']['confirmation_confidence_upper_lim'], 1000)
        self.assertEqual(runconfig['algo_config']['confirmation_confidence_threshold'], 22.5)
        self.assertEqual(runconfig['algo_config']['metric_value_upper_lim'], 10.0)
        self.assertEqual(runconfig['algo_config']['model_source'], 'transformer_optimized')
        self.assertIsNone(runconfig['algo_config']['model_cfg_path'])
        self.assertIsNone(runconfig['algo_config']['model_wts_path'])
        self.assertEqual(runconfig['algo_config']['model_dtype'], 'float32')
        self.assertFalse(runconfig['algo_config']['use_date_encoding'])
        self.assertEqual(runconfig['algo_config']['n_anniversaries_for_mw'], 3)
        self.assertEqual(runconfig['algo_config']['delta_window_days'], 60)
        self.assertEqual(runconfig['algo_config']['model_context_length'], 10)
