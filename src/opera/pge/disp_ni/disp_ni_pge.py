...
from opera.util.h5_utils import get_cslc_ni_product_metadata
from opera.util.h5_utils import get_disp_ni_product_metadata
...
class DispNIPostProcessorMixin(PostProcessorMixin):
    ...
    def _validate_output(self):
        ...
        if product_type == 'DISP_NI_FORWARD' and len(nc_files) > 1:
    ...
    def _core_filename(self, inter_filename=None):
        ...
        # Mode: NI-A/B acquisition mode, should be fixed to "IW"
        mode = "IW"
        ...
        for cslc_file in cslc_file_list:
            try:
                cslc_metadata = get_cslc_ni_product_metadata(abspath(cslc_file))
    ...
    def _browse_filename(self, inter_filename):
        ...
        The browse image filename for the DISP-NI PGE consists of:

            <Core filename>.png

        Where <Core filename> is returned by DispNIPostProcessorMixin._core_filename()
    ...
    def _netcdf_filename(self, inter_filename):
        """
        Returns the file name to use for netCDF files produced by the DISP-NI PGE.

        The netCDF filename for the DISP-NI PGE consists of:
    ...
    def _catalog_metadata_filename(self):
        """
        Returns the file name to use for Catalog Metadata produced by the DISP-NI PGE.

        The Catalog Metadata file name for the DISP-NI PGE consists of:

            <Ancillary filename>.catalog.json

        Where <Ancillary filename> is returned by DispNIPostProcessorMixin._ancillary_filename()
    ...
    def _iso_metadata_filename(self, inter_disp_product_filename):
        """
        Returns the file name to use for ISO Metadata produced by the DISP-NI PGE.

        The ISO Metadata file name for the DISP-NI PGE consists of:

            <DISP filename>.iso.xml

        Where <DISP filename> is returned by DispNIPostProcessorMixin._core_filename()

        Parameters
        ----------
        inter_disp_product_filename : str
            The DISP-NI product intermediate file name used to look-up the
            full filename assigned to the DISP-NI output products.
    ...
    def _log_filename(self):
        """
        Returns the file name to use for the PGE/SAS log file produced by the DISP-NI PGE.

        The log file name for the DISP-NI PGE consists of:
    ...
    def _qa_log_filename(self):
        ...
        file produced by the DISP-NI PGE.
    ...
    def _collect_disp_ni_product_metadata(self, disp_product):
        ...
        output_product_metadata = get_disp_ni_product_metadata(disp_product)
    ...
    def _create_custom_metadata(self):
        ...
        custom_metadata = {
            ...
            'ISO_OPERA_InstrumentKeywords': ['NI']
        }
    ...
    def _create_iso_metadata(self, disp_metadata):
        """
        Creates a rendered version of the ISO metadata template for DISP-NI
        ...
        ----------
        disp_metadata : dict
            The product metadata corresponding to the DISP-NI product to generate
            the corresponding ISO xml for.

        Returns
        -------
        rendered_template : str
            The ISO metadata template for DISP-NI filled in with values from
    ...
    def _stage_output_files(self):
        ...
        each set of DISP-NI products created from the set of input CSLCs, since
        ...
        # For CSLC-NI, each burst-based product gets its own ISO xml
...
class DispNIExecutor(DispNIPreProcessorMixin, DispNIPostProcessorMixin, PgeExecutor):
    ...
    SAS_VERSION = "0.1.0"  # Beta release https://github.com/opera-adt/disp-ni/releases/tag/v0.1.0
