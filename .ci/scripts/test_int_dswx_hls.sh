...
while [[ $# -gt 0 ]]; do
  case $1 in
    -h|--help)
      echo "Usage: test_int_dswx_hls.sh [-h|--help] [-p|--pge-image <image name>] [-t|--tag <tag>] [-d|--testdata <testdata .tgz file>]"
      exit 0
      ;;
    -p|--pge-image)
      PGE_IMAGE=$2
...
do
    ...
    docker run --rm -u conda:conda -v $(pwd):/home/conda/runconfig:ro \
                     -v $(pwd)/${DATA_DIR}/input_dir:/home/conda/input_dir:ro \
                     -v $(pwd)/$OUTPUT_DIR:/home/conda/output_dir -i --tty \
                     ${PGE_IMAGE}:${TAG} --file /home/conda/runconfig/dswx_hls.yaml
            ...
            if [ -f "${EXPECTED_DIR}/${output_file##*/}" ]; then
                ...
                then
                    docker_out=$(docker run --rm -u conda:conda \
                                            ...
                                            --entrypoint python3 opera/proteus:beta \
