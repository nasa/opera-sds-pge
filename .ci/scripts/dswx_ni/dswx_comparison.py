...
def _get_dswx_band_index_from_filename(path):
    filename = os.path.basename(path)
    band_no = int(filename.split('_')[FILENAME_BAND_FIELD_INDEX][1:])
    return band_no


def _print_first_value_diff(image_1, image_2, prefix, tolerance=COMPARE_DSWX_SAR_PRODUCTS_ERROR_TOLERANCE):
    """
    ...
    for i in range(image_1.shape[0]):
        for j in range(image_1.shape[1]):
            if abs(image_1[i, j] - image_2[i, j]) >= tolerance:
...
def compare_dswx_sar_products(file_1, file_2):
    ...
    dswx_band_1 = _get_dswx_band_index_from_filename(file_1)
    dswx_band_2 = _get_dswx_band_index_from_filename(file_2)

    if dswx_band_1 != dswx_band_2:
        print(f'[FAIL] Files seem to correspond to different DSWx product bands')
        return False

    tolerance = BAND_SPECIFIC_TOLERANCES.get(dswx_band_1, COMPARE_DSWX_SAR_PRODUCTS_ERROR_TOLERANCE)
    ...
    for b in range(1, nbands_1 + 1):
        ...
        if not flag_bands_are_equal:
            _print_first_value_diff(image_1, image_2, prefix, tolerance=tolerance)
