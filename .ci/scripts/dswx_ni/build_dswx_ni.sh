...
Building DSWx-NI PGE docker image...
...
[ -z "${TAG}" ] && TAG="${USER}-dev"
[ -z "${SAS_IMAGE}" ] && SAS_IMAGE="artifactory-fn.jpl.nasa.gov:16001/gov/nasa/jpl/opera/adt/opera/dswx-ni:gamma_patch_0.3.1"
...
docker build ${PLATFORM} --progress plain --rm --force-rm -t ${IMAGE}:${TAG} \
    ...
    --file ${WORKSPACE}/.ci/docker/Dockerfile_dswx_ni ${WORKSPACE}

echo "DSWx-NI PGE Docker image build complete"
