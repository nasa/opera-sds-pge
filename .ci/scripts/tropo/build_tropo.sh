...
Building TROPO PGE docker image...
...
[ -z "${TAG}" ] && TAG="${USER}-dev"
[ -z "${SAS_IMAGE}" ] && SAS_IMAGE="artifactory-fn.jpl.nasa.gov:16001/gov/nasa/jpl/opera/adt/opera/tropo:interface_0.1"
...
docker build ${PLATFORM} --progress plain --rm --force-rm -t ${IMAGE}:${TAG} \
    ...
    --file ${WORKSPACE}/.ci/docker/Dockerfile_tropo ${WORKSPACE}

echo "TROPO PGE Docker image build complete"
