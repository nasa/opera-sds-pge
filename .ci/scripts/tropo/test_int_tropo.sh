#!/bin/bash
# Script to execute integration tests on OPERA TROPO PGE Docker image
#
set -e
umask 002
...
echo '
================================================
Integration Testing TROPO PGE docker image...
================================================
'
...
# INPUT/OUTPUT_DATA should be the name of the corresponding archives in s3://operasds-dev-pge/tropo/
# RUNCONFIG should be the name of the runconfig file  in <repo_root>/.ci/scripts/tropo/
[ -z "${WORKSPACE}" ] && WORKSPACE=$(realpath "$(dirname "$(realpath "$0")")"/../../..)
[ -z "${PGE_TAG}" ] && PGE_TAG="${USER}-dev"
[ -z "${INPUT_DATA}" ] && INPUT_DATA="tropo_calval_0.2_expected_input.zip "
[ -z "${EXPECTED_DATA}" ] && EXPECTED_DATA="tropo_calval_0.2_expected_output.zip "
[ -z "${RUNCONFIG}" ] && RUNCONFIG="opera_pge_tropo_0.2_calval_runconfig.yaml"
...
output_dir="${TMP_DIR}/output_tropo"
...
# the testdata reference metadata contains this path so we use it here
scratch_dir="${TMP_DIR}/tropo_scratch/scratch_dir"
