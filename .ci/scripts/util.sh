...
metrics_collection_start ()
{
    ...
    # miscellaneous statistics will be added in a future release
...
#    echo "SECONDS,disk_used,swap_used,total_threads,last_line" > $metrics_misc
    ...
    # For the time being (may ultimately remove) take out the sleep ("$(metrics_seconds),$ds" >> ${metrics_stats}; sleep .1; done })
...
metrics_collection_end ()
{
    container_name=$1
    exit_code=$2
#  The variables below will be used in the future.
#    mce_pge=$3
#    mce_runconfig=$4
    ...
    metrics_stats="${container_name}_metrics_stats.csv"
    # for the time being taking out miscellaneous statistics
#    metrics_misc="${container_name}_metrics_misc.csv"
...
#    rm "${container_name}"_metrics_misc_bg_pid.txt
    ...
    then
        ...
        python3 ./process_metric_data.py $metrics_stats
