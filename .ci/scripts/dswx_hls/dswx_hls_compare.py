...
def compare_dswx_hls_products(file_1, file_2, metadata_exclude_list):
    ...
    if not flag_same_nbands:
        print(f'{prefix}* input 1 has {nbands_1} bands and input 2 '
        ...
        for b in range(1, nbands_1 + 1):
            ...
            if num_image_differences > 0:
                ...
                details += (f"{prefix}* image band {b} difference count is {num_image_differences} "
                ...
                details += (f'{prefix}* e.g. image band {b} input 1 has value "{image_1[i, j]}" in position '
                            f'(i: {i}, j: {j}) whereas input 2 has value "{image_2[i, j]}" '
                            'in the same position.')
            else:
                flag_bands_are_equal = True

            flag_all_ok, flag_bands_are_equal_str = _get_prefix_str(flag_bands_are_equal, flag_all_ok)
            print(f'{flag_bands_are_equal_str}Comparing Band {b} - '
    ...
    if not flag_same_geotransforms:
        print(f'{prefix}* input 1 geotransform with content "{geotransform_1}" '
    ...
    if metadata_compare_message:
        print(prefix + metadata_compare_message)
