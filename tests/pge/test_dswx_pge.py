...
class DSWxPgeTestCase(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.working_dir = tempfile.TemporaryDirectory(prefix="test_dswx_pge_", suffix="_temp", dir=os.curdir)
        ...
        cls.input_file = tempfile.NamedTemporaryFile(dir=input_dir, prefix="test_input", suffix=".tif")
    ...
    def test_dswx_pge_input_validation(self):
        ...
        self.assertIn(
            f"Could not locate specified input file/directory {abspath('non_existent_file.tif')}", log_contents
        )
        ...
        self.assertIn(
            f"Input directory {abspath('dswx_pge_test/scratch_dir')} does not contain any tif files", log_contents
        )
        ...
        self.assertIn(f"Input file {abspath(runconfig_path)} does not have .tif extension", log_contents)
    ...
    def test_dswx_pge_output_validation(self):
        ...
        self.assertIn(f"Expected SAS output file {abspath(expected_output_file)} does not exist", log_contents)
        ...
        self.assertIn(f"SAS output file {abspath(expected_output_file)} was created but is empty", log_contents)
